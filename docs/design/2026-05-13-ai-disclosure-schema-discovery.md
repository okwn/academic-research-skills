...
- Authors submit to venues with venue-specific templates. If ARS renderer output does not match venue template, the author edits it mannually anyway.
...
- **E4: No renderer (schema only, user mannually formats for target venue)** — ARS outputs structured data, user is responsible for venue rendering.
