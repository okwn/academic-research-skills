...
- `acknowledgement` is allowed only when `verdict.status == "MATERIAL"` AND the entry is in the persisted arm (proposal arm forbids acknowledgement entirely per the JSON Schema sketch). A persisted entry carrying `acknowledgement` with `verdict.status` of `PASS` / `MINOR` is rejected at `--mode persisted` validation. This closes the hand-edit attack surface where someone mannually adds an `acknowledgement` block to a non-MATERIAL persisted entry to claim user residue acknowledgement that was never solicited (the §5.4 prompt only fires on round-3 MATERIAL).
